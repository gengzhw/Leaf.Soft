    ...
    public class XlsHelper : IXlsHelper
    {
        ...
        public IList<Qichacha> LoadFromXls(string filename)
        {
            ...
            for (i = 1; i <= num; i++)
            {
                ...
                string name = XlsCellHeler.getCellValue(hssfRow.GetCell(0));
                string linkMan = XlsCellHeler.getCellValue(hssfRow.GetCell(1));
                string fundDate = XlsCellHeler.getCellValue(hssfRow.GetCell(2));
                string regCapitalStr = XlsCellHeler.getCellValue(hssfRow.GetCell(3));
                string actWorkAddr = XlsCellHeler.getCellValue(hssfRow.GetCell(4));
                string email = XlsCellHeler.getCellValue(hssfRow.GetCell(5));
                string linkPhone = XlsCellHeler.getCellValue(hssfRow.GetCell(6));
                string actScope = XlsCellHeler.getCellValue(hssfRow.GetCell(7));
                string webSiteUrl = XlsCellHeler.getCellValue(hssfRow.GetCell(8));

                qilist.Add(new Qichacha() { Name=name,LinkMan=linkMan,FundDate=fundDate,RegCapitalStr=regCapitalStr,
                    ActWorkAddr =actWorkAddr,Email=email,LinkPhone=linkPhone,ActScope=actScope,WebSiteUrl=webSiteUrl });
            }
